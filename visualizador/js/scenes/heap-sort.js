...
const STRINGS = {
  en: {
    ready: 'Ready to play.',
    buildPhase: 'Phase 1 — building the max-heap: every champion bubbles up so the strongest reaches the top 🏆',
    extractPhase: 'Phase 2 — extracting champions: the top fighter drops into the sorted tail, one by one.',
    ...
    settled: (value) =>
      `${tag(value)} lands in the sorted tail — its final place ✔`,
  ...
  es: {
    ...
    buildPhase: 'Fase 1 — armando el max-heap: cada campeón burbujea hacia arriba para que el más fuerte llegue a la cima 🏆',
    extractPhase: 'Fase 2 — extrayendo campeones: el de arriba cae a la cola ordenada, uno por uno.',
    siftCompare: (parent, child) =>
      `Duelo: padre ${tag(parent)} vs hijo más fuerte ${tag(child)}.`,
    siftCompareOk: (parent, child) =>
    ...
    settled: (value) =>
      `${tag(value)} aterriza en la cola ordenada — su lugar definitivo ✔`,
...
export default function mountHeapSort(host, meta) {
  ...
  function clearHi() {
    nodes.forEach((n) =>
      n.classList.remove('hs-parent', 'hs-child', 'hs-swap', 'hs-extract')
    );
  }
  ...
  function dropSorted(index, value) {
    ...
    const after = Array.from(sortedRow.children).find(
      (c) => Number(c.dataset.idx) > index
    );
